 ...
 */
 ...
 const APIHelper = {
     ...
     */
     appendUrlWithTemplateParameters(queryBuilder, parameters) {
        ...
        function replace(param) {
            ...
            queryString = queryBuilder.replace(`{${(parameters.indexOf(param))}}`, replaceValue);
        }
        parameters.every(replace);
     ...
     */
     formatString(str, arguments) {
        const args = arguments;
        let formattedStr = str;
        if (!formattedStr) return formattedStr;
        for (let iter = 1; iter < args.length; iter = iter + 1) {
            const reg = new RegExp('\\{' + (iter - 1) + '\\}', 'gm');
            formattedStr = formattedStr.replace(reg, args[iter]);
        }
        return formattedStr;
    },
